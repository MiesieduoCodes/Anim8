          ...
          <div className="px-4 py-3 cursor-pointer text-left">
            ...
            >
              ...
              {link.submenu && (
                <span className="ml-2">
                  <svg
                    className={`w-6 h-6 transform ${openIndex === index ? "rotate-180" : "rotate-0"} transition-transform duration-300`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
            ...
            {link.submenu && (
              <div
                className={`bg-white shadow-lg rounded-lg py-4 px-6 transition-all duration-300 transform ${
                  openIndex === index ? "opacity-100 scale-100 visible" : "opacity-0 scale-95 invisible"
                } md:absolute md:left-0 md:top-20 md:shadow-md md:w-64 w-full`}
              >
                <ul className="space-y-2">
                  {link.sublinks.map((sublink, subIndex) => (
                    <li key={subIndex} className="text-sm text-gray-600 hover:text-blue-500 transition duration-300">
